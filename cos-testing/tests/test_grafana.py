...
def grafana() -> Grafana:
    ...
    for method_name in _RETRIED_METHODS:
        getattr(g, method_name).retry.wait = wait_none()
        getattr(g, method_name).retry.stop = lambda *a, **kw: True
    g.session = MagicMock()
    return g
...
class TestInit:
    ...
    def test_custom_timeout(self) -> None:
        ...
        for method_name in _RETRIED_METHODS:
            stop = getattr(g, method_name).retry.stop
...
class TestHasDatasource:
    ...
    def test_no_args_empty_list(self, grafana: Grafana, mock_get: MagicMock) -> None:
        mock_get.return_value = _ok_response([])

        assert grafana.has_datasource() is False
