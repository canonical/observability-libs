...
class Pyroscope:
    ...
    def __post_init__(self) -> None:
        ...
        self.render = self._retry_on_error(self.render)
        self.get_profile_types = self._retry_on_error(self.get_profile_types)
    ...
    def render(
        ...
        if end:
            params["until"] = end
        resp = self.session.get(f"{self.url}/pyroscope/render", params=params)
    ...
    def get_profile_types(self) -> dict:
        ...
        resp = self.session.post(
            f"{self.url}/querier.v1.QuerierService/ProfileTypes",
            json={},
    ...
    def get_labels(self) -> dict:
        ...
        resp = self.session.post(
            f"{self.url}/querier.v1.QuerierService/LabelNames",
            json={},
            headers={"Content-Type": "application/json"},
        )
        resp.raise_for_status()
    ...
    def get_label_values(self, label: str) -> list:
        ...
        resp = self.session.get(
            f"{self.url}/pyroscope/label-values",
            params={"label": label},
        )
        resp.raise_for_status()
        return resp.json()
