...
class TestInit:
    ...
    def test_custom_timeout(self):
        ...
        assert client._retry_stop.max_delay == 30
...
class TestQueryRange:
    def test_minimal_params(self, loki, mock_get):
        ...
        mock_get.assert_called_once_with(
            ...
            params={"query": '{job="bar"}', "limit": 1000},
        )
        assert result == payload
...
class TestIsReady:
    def test_ready(self, loki, mock_get):
        mock_get.return_value = _json_response({}, status_code=200)
        assert loki.is_ready() is True
        mock_get.assert_called_once_with(f"{BASE_URL}/ready")

    def test_not_ready(self, loki, mock_get):
        mock_get.return_value = _json_response({}, status_code=503)
        assert loki.is_ready() is False

    def test_connection_error(self, loki, mock_get):
        mock_get.side_effect = requests.ConnectionError("connection refused")
        assert loki.is_ready() is False

    def test_timeout(self, loki, mock_get):
