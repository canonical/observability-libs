...
class TestIsReady:
    def test_ready(self, mimir: Mimir):
        mimir.session.get.return_value = _mock_response({}, status_code=200)
        assert mimir.is_ready() is True
        mimir.session.get.assert_called_once_with("http://mimir:9009/ready")

    def test_not_ready(self, mimir: Mimir):
        mimir.session.get.return_value = _mock_response({}, status_code=503)
        assert mimir.is_ready() is False

    def test_connection_error(self, mimir: Mimir):
        mimir.session.get.side_effect = requests.ConnectionError("connection refused")
        assert mimir.is_ready() is False

    def test_timeout(self, mimir: Mimir):
        mimir.session.get.side_effect = requests.Timeout("timed out")
        assert mimir.is_ready() is False
