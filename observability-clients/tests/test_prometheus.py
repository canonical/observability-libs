...
class TestInit:
    ...
    def test_custom_timeout(self):
        ...
        assert client._retry_stop.max_delay == 120
...
class TestIsReady:
    def test_ready(self, prom):
        prom.session.get.return_value = _mock_response({}, status_code=200)
        assert prom.is_ready() is True
        prom.session.get.assert_called_once_with("http://localhost:9090/-/ready")

    def test_not_ready(self, prom):
        prom.session.get.return_value = _mock_response({}, status_code=503)
        assert prom.is_ready() is False

    def test_connection_error(self, prom):
        prom.session.get.side_effect = requests.ConnectionError("connection refused")
        assert prom.is_ready() is False

    def test_timeout(self, prom):
        prom.session.get.side_effect = requests.Timeout("timed out")
        assert prom.is_ready() is False

    def test_not_retried_on_error(self, prom):
        prom.session.get.side_effect = requests.ConnectionError("connection refused")
        prom.is_ready()
        assert prom.session.get.call_count == 1
